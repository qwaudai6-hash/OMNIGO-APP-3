...
func (r *WishlistRepository) ToggleFavorite(ctx context.Context, customerTrackingID, productTrackingID string) (bool, error) {
	// The favorites table uses `customer_tracking_id` (per migration 0001_init.sql),
	// not `customer_tracking_id`. The Go code historically used the wrong
	// column name, so all wishlist writes silently no-op'd.
	var exists bool
	checkQuery := `SELECT EXISTS(SELECT 1 FROM favorites WHERE customer_tracking_id = $1 AND product_tracking_id = $2)`
	...
	if exists {
		// Remove
		_, err := r.writer.Exec(ctx, `DELETE FROM favorites WHERE customer_tracking_id = $1 AND product_tracking_id = $2`, customerTrackingID, productTrackingID)
		return false, err
	}
	...
	// Add
	_, err = r.writer.Exec(ctx, `INSERT INTO favorites (customer_tracking_id, product_tracking_id) VALUES ($1, $2)`, customerTrackingID, productTrackingID)
	return true, err
}
...
func (r *WishlistRepository) ListFavorites(ctx context.Context, customerTrackingID string) ([]string, error) {
	rows, err := r.reader.Query(ctx, `SELECT product_tracking_id FROM favorites WHERE customer_tracking_id = $1 ORDER BY created_at DESC`, customerTrackingID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *WishlistRepository) IsFavorite(ctx context.Context, customerTrackingID, productTrackingID string) (bool, error) {
	var exists bool
	err := r.reader.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM favorites WHERE customer_tracking_id = $1 AND product_tracking_id = $2)`, customerTrackingID, productTrackingID).Scan(&exists)
	return exists, err
}
...
func (r *WishlistRepository) RemoveFavorite(ctx context.Context, customerTrackingID, productTrackingID string) error {
	res, err := r.writer.Exec(ctx, `DELETE FROM favorites WHERE customer_tracking_id = $1 AND product_tracking_id = $2`, customerTrackingID, productTrackingID)
	if err != nil {
		return err
	}

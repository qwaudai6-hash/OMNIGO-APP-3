...
until docker exec "$(docker compose -f "$COMPOSE_FILE" ps -q omnigo-postgres)" pg_isready -U omnigo_user -d omnigo_db >/dev/null 2>&1; do
  echo -n "."
  sleep 2
done
...
curl -s -X POST -H "Accept:application/json" -H "Content-Type:application/json" \
  http://localhost:8083/connectors/ -d '{
    ...
    "config": {
      ...
      "database.hostname": "omnigo-postgres",
      "database.port": "5432",
      "database.user": "omnigo_user",
      "database.password": "omnigo_password",
      "database.dbname": "omnigo_db",
      "database.server.name": "omnigo",
